...
def sync_toolbar() -> None:
    ...
    if "const ALL: [Self; 12]" in text:
        ...
        raise SystemExit("toolbar count: expected 12 or 13 tools")
    ...
    text = replace_once(text, '            Self::Select => "1",\n            Self::Hand => "H",', '            Self::Select => "1",\n            Self::Lasso => "",\n            Self::Hand => "H",', "toolbar Lasso display shortcut")
    text = replace_once(text, '            Self::Select => matches!(value.as_str(), "v" | "1"),\n            Self::Hand => value == "h",', '            Self::Select => matches!(value.as_str(), "v" | "1"),\n            Self::Lasso => false,\n            Self::Hand => value == "h",', "toolbar Lasso shortcut matching")
...
#[cfg(test)]
mod drawing_toolbar_lasso_tests {
    ...
    fn main_toolbar_exposes_frame_and_lasso_tools() {
        ...
        assert_eq!(DrawingTool::Lasso.shortcut(), "");
        assert!(!DrawingTool::Lasso.matches_shortcut("v"));
        assert!(DrawingTool::Select.matches_shortcut("v"));
        assert!(DrawingTool::Select.matches_shortcut("1"));
    }
}
'''
