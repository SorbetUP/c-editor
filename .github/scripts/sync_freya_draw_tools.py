...
def sync_toolbar() -> None:
    ...
    text = replace_once(
        ...
        '            Self::Select => "1",\n            Self::Hand => "H",',
        '            Self::Select => "1",\n            Self::Lasso => "V",\n            Self::Hand => "H",',
        "toolbar Lasso shortcut",
    )
    text = replace_once(
        text,
        '            Self::Select => matches!(value.as_str(), "v" | "1"),\n            Self::Hand => value == "h",',
        '            Self::Select => matches!(value.as_str(), "v" | "1"),\n            Self::Lasso => false,\n            Self::Hand => value == "h",',
        "toolbar Lasso keyboard matcher",
    )
    select_icon = '''            Self::Select => br#"<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m5 3 14 9-6 2-2 6Z"/></svg>"#,
            Self::Hand =>'''
    lasso_icon = '''            Self::Select => br#"<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m5 3 14 9-6 2-2 6Z"/></svg>"#,
            Self::Lasso => br#"<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><ellipse cx="12" cy="8" rx="7" ry="5"/><path d="M12 13c0 5 7 3 7 7 0 1-1 2-3 2"/></svg>"#,
            Self::Hand =>'''
    text = replace_once(text, select_icon, lasso_icon, "toolbar Lasso icon")
...
def sync_canvas() -> None:
    ...
    text = replace_once(
        ...
        '#[path = "drawing_text_edit.rs"]\nmod drawing_text_edit;\n#[path = "drawing_transform.rs"]\nmod drawing_transform;',
    ...
    text = replace_once(
        ...
        "canvas Lasso pointer down",
    )
    text = replace_once(
        text,
        "                DrawingTool::Selection | DrawingTool::Hand | DrawingTool::Eraser\n            ) {",
        "                DrawingTool::Selection\n                    | DrawingTool::Lasso\n                    | DrawingTool::Hand\n                    | DrawingTool::Eraser\n            ) {",
        "canvas Lasso pointer move",
    )
    text = replace_once(
        text,
        "        .children(primitives)\n        .maybe_child(editing_index.map(|index| {",
        "        .children(primitives)\n        .maybe_child((!snapshot.selected_element_ids().is_empty()).then(|| {\n            drawing_transform::TransformHandles { canvas: state }.into()\n        }))\n        .maybe_child(editing_index.map(|index| {",
    ...
    text = replace_once(
        ...
        "canvas Lasso palette",
    )
    text = replace_once(
        text,
        "        DrawingTool::Selection => elephant_draw::DrawingTool::Selection,\n        DrawingTool::Hand",
        "        DrawingTool::Selection | DrawingTool::Lasso => elephant_draw::DrawingTool::Selection,\n        DrawingTool::Hand",
        "canvas Lasso core mapping",
    )
    path.write_text(text)
...
def sync_scene() -> None:
    ...
    text = replace_once(
        ...
        "    BoxSelect {\n        start_world: [f32; 2],\n        current_world: [f32; 2],\n    },\n    Pan {",
        "    BoxSelect {\n        start_world: [f32; 2],\n        current_world: [f32; 2],\n    },\n    LassoSelect {\n        points: Vec<[f32; 2]>,\n    },\n    Pan {",
        "scene Lasso interaction",
    )
    text = replace_once(
        text,
        '            "Hand" => "hand",\n            "Frame" => "frame",',
        '            "Hand" => "hand",\n            "Lasso" => "lasso",\n            "Frame" => "frame",',
        "scene Lasso tool mapping",
    )
...
def sync_render_svg() -> None:
    ...
    for (index, [x, y]) in points.iter().enumerate() {
        ...
        let _ = std::fmt::Write::write_fmt(
            &mut path,
            format_args!("{command} {} {} ", x - min_x, y - min_y),
        );
        signature = signature
    ...
    padded_surface(
        ...
        (max_y - min_y).max(1.0),
        content,
        "Drawing lasso selection",
        4.0,
    )
}

'''
    text = text.replace(anchor, lasso + anchor, 1)
    path.write_text(text)
