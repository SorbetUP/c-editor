...
- `MarkdownEditorApp/` provides the macOS shell built by the root `Makefile`; top-level `*.m`/`*.h` Objective-C files are legacy app pieces and should not receive new generated artifacts.
- `tools/` contains developer tooling (`tui/`, `debug/`, `build/`) that link against the engines for interactive and scripted trials.
- `web/site/` is the Pages source tree, `dist/github-pages/` is the publishable artifact, and `web/wasm/` holds browser bindings; `flutter/` covers the Dart front end.
- Treat `versions/` and `old_builds/` as archives, not as the canonical path for new feature work.
...
- `./scripts/build_github_pages.sh && ./scripts/smoke_test_github_pages.sh` is the canonical Pages validation path before a push that touches web/WASM.
...
- Keep headers self-contained with include guards and favor `static` helpers for internal utilities; never commit generated `*.o`, `.wasm`, bundle artifacts, or root-level binary outputs.
