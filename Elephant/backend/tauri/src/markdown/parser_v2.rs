...
pub fn parse_markdown_document(markdown: &str) -> MarkdownDocument {
  ...
  let outline = extract_outline_cmark(body);
  let links = extract_links_cmark(body);
  let images = extract_images_cmark(body);
...
pub fn extract_outline_cmark(markdown: &str) -> Vec<MarkdownHeading> {
...
pub fn extract_links_cmark(markdown: &str) -> Vec<MarkdownLink> {
  let parser = Parser::new_ext(markdown, cmark_options());
...
pub fn extract_images_cmark(markdown: &str) -> Vec<MarkdownImage> {
  let parser = Parser::new_ext(markdown, cmark_options());
