...
pub async fn list_codex_models(payload: &Value) -> R<Value> {
    request_json(
        codex_models_url(&pi_base_url(payload), &codex_client_version(payload)),
        payload,
    )
    .await
}
...
pub async fn chat_completion(model: &str, messages: &[Value], payload: &Value) -> R<PiChatResult> {
    ...
    let api_key = pi_api_key(payload);
    let response = with_pi_headers(
        client()?.post(chat_completions_url(&base_url)).json(&body),
        &api_key,
    )
    .send()
    .await
    ...
    let data = response.json::<Value>().await.unwrap_or_else(|_| json!({}));
    if !status.is_success() {
        return Err(pi_error_text(
            &data,
            format!("PI chat/completions returned HTTP {status}."),
        ));
    }
    let answer = data
...
mod tests {
    ...
    fn builds_pi_openai_and_codex_model_urls() {
        assert_eq!(
            models_url("http://127.0.0.1:8317/v1"),
            "http://127.0.0.1:8317/v1/models"
        );
        assert_eq!(
            codex_models_url("http://127.0.0.1:8317/v1", "elephantnote"),
            "http://127.0.0.1:8317/v1/models?client_version=elephantnote"
        );
    }

    #[test]
    fn normalizes_openai_compatible_model_list() {
        let ids = normalize_openai_model_ids(
            &json!({ "object": "list", "data": [{ "id": "gpt-5.5", "object": "model" }] }),
        );
    ...
    fn normalizes_codex_client_model_list() {
        let ids = normalize_codex_model_slugs(
            &json!({ "models": [{ "slug": "gpt-5.5", "display_name": "GPT-5.5" }] }),
        );
        assert_eq!(ids, vec!["gpt-5.5".to_string()]);
    }
}
