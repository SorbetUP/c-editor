...
impl LaserTrail {
    ...
    pub fn add_point(&mut self, position: [f32; 2], timestamp_ms: f64) -> bool {
        if self
            .points
            .last()
            .is_some_and(|point| point.position == position)
        {
            return false;
        }
    ...
    pub fn visible_samples(&self, now_ms: f64) -> Vec<LaserSample> {
        let points = self.streamlined_points();
        let total = points.len();
        points
            .iter()
            .enumerate()
            .filter_map(|(index, point)| {
                let size = LASER_SIZE * size_mapping(point.timestamp_ms, index, total, now_ms);
                (size > 0.0).then_some(LaserSample {
    ...
    pub fn stroke_outline(&self, now_ms: f64, size_override: Option<f32>) -> Vec<[f32; 2]> {
        ...
        let get_size = |point: WorkPoint, index: usize, running_length: f32| {
            let _ = running_length;
        ...
        if len == 2 {
            ...
            let mut output = circle_points(c.position, c_size, p_angle, std::f32::consts::PI + p_angle);
        ...
        for index in 1..len - 1 {
            ...
            let pa_pc = add(
                c.position,
                scale(if magnitude(ft_dir) == 0.0 { dir_pc } else { normalize(ft_dir) }, c_size),
            );
            let pa_nc = add(
                c.position,
                scale(if magnitude(bt_dir) == 0.0 { dir_nc } else { normalize(bt_dir) }, c_size),
            );

            let c_angle = norm_angle(angle(c.position, p.position, n.position));
            let variance = if speed > 35.0 { 0.5 } else { 1.0 };
            let d_angle = CORNER_DETECTION_MAX_ANGLE_DEGREES.to_radians() * variance;
        ...
        if visible_start_index >= len - 2 {
            if self.keep_head {
                let c = points[len - 1];
                let mut output = circle_points(c.position, LASER_SIZE, 0.0, std::f32::consts::TAU);
        ...
        if start_cap_size > 0.1 {
            for step in 0..=16 {
                ...
                start_cap.insert(
                    0,
                    add(first.position, rotate(scale(ppdir_fs, start_cap_size), -theta)),
                );
            }
    ...
    pub fn is_visible(&self, now_ms: f64) -> bool {
        !self.stroke_outline(now_ms, None).is_empty()
    }

    fn streamlined_points(&self) -> Vec<WorkPoint> {
...
pub struct LaserTrails {
    current: Option<LaserTrail>,
    past: Vec<LaserTrail>,
}

impl LaserTrails {
    pub fn current(&self) -> Option<&LaserTrail> {
        self.current.as_ref()
    }

    pub fn past(&self) -> &[LaserTrail] {
        &self.past
    }
    ...
    pub fn start_path(&mut self, x: f32, y: f32, timestamp_ms: f64) {
        self.current = Some(LaserTrail::new([x, y], timestamp_ms));
    }

    pub fn add_point_to_path(&mut self, x: f32, y: f32, timestamp_ms: f64) -> bool {
        self.current
            .as_mut()
            .is_some_and(|trail| trail.add_point([x, y], timestamp_ms))
    }

    pub fn end_path(&mut self) -> bool {
        let Some(mut trail) = self.current.take() else {
            return false;
        };
        trail.close();
        self.past.push(trail);
        true
    }
    ...
    pub fn visible_samples(&self, now_ms: f64) -> Vec<LaserSample> {
        self.past
            .iter()
            .flat_map(|trail| trail.visible_samples(now_ms))
            .chain(
                self.current
                    .iter()
                    .flat_map(|trail| trail.visible_samples(now_ms)),
    ...
    pub fn stroke_outlines(&self, now_ms: f64, size_override: Option<f32>) -> Vec<Vec<[f32; 2]>> {
        self.past
            .iter()
    ...
    pub fn prune(&mut self, now_ms: f64) {
        self.past.retain(|trail| trail.is_visible(now_ms));
    }
}
...
fn size_mapping(timestamp_ms: f64, index: usize, total: usize, now_ms: f64) -> f32 {
    ...
    let remaining_including_current = total.saturating_sub(index);
    let length = (LASER_DECAY_LENGTH
        .saturating_sub(remaining_including_current.min(LASER_DECAY_LENGTH))) as f32
...
fn circle_points(center: [f32; 2], radius: f32, start: f32, end: f32) -> Vec<[f32; 2]> {
    ...
    (0..=steps)
        .map(|step| {
            ...
            [center[0] + theta.cos() * radius, center[1] + theta.sin() * radius]
        })
        .collect()
}
