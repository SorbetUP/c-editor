        ...
        rect()
            ...
            .on_global_pointer_move(move |event: Event<PointerEventData>| {
                ...
                let inside = point.x >= f64::from(area.min_x())
                    ...
                    && point.y >= f64::from(area.min_y())
                    && point.y <= f64::from(area.max_y());
                let current = hover_state.read().hovered_target.clone();
                if inside {
                    if current.as_deref() != Some(move_key.as_str()) {
                        hover_state.write().set_hovered_target(move_key.clone());
                    }
                    hover_state
                        .write()
