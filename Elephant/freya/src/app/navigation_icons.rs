...
mod tests {
    ...
    #[test]
    fn sidebar_icons_match_the_lucide_geometry_used_by_vue() {
        let neutral = std::str::from_utf8(source(Icon::PanelLeft)).unwrap();
        let down = std::str::from_utf8(source(Icon::ChevronDown)).unwrap();
        assert!(neutral.contains("M9 3v18"));
        assert!(down.contains("m6 9 6 6 6-6"));
    }
}
