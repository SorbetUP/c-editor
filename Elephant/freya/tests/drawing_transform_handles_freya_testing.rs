...
fn selected_element_can_be_resized_and_rotated_with_native_handles() {
    ...
    let southeast = node(&runner, "Drawing transform handle se");
    ...
    let rotate = node(&runner, "Drawing transform handle rotation");
    ...
    runner.move_cursor((
        f64::from(center.0 + bounds.2 / 2.0 + 30.0),
        f64::from(center.1),
    ));
    ...
    assert!(rotated.angle.abs() > 0.25, "rotation handle must update the Excalidraw angle");
