...
export class MuyaRustProtocolError extends Error {
  constructor(payload) {
    super(payload?.message || 'Muya Rust rejected the editor command.')
    this.name = 'MuyaRustProtocolError'
    this.code = payload?.code || 'unknown'
  }
}

export class MuyaRustPatchError extends Error {
  constructor(cause) {
    super(`Unable to apply Muya Rust patches: ${cause?.message || String(cause)}`)
    this.name = 'MuyaRustPatchError'
    this.cause = cause
  }
}
...
export class MuyaRustBridge {
  constructor(engine, options = {}) {
    if (!engine || typeof engine.handle_json !== 'function') {
      throw new TypeError('Muya Rust engine must expose handle_json(request).')
    }
    if (typeof engine.snapshot_json !== 'function') {
      throw new TypeError('Muya Rust engine must expose snapshot_json().')
    }
  ...
  async _dispatch(command) {
    if (this.desynchronized) {
      throw new MuyaRustPatchError(new Error('Bridge is desynchronized; call recover() first.'))
    }
    ...
    if (response.type === 'error') {
      const error = new MuyaRustProtocolError(response.payload)
      this.onError(error)
      throw error
    }
  ...
  async _snapshot() {
    ...
    if (response.type !== 'snapshot') {
      throw new TypeError(`Expected a Muya Rust snapshot, received ${response.type}.`)
    }
    return this._acceptSnapshot(response.payload)
  }
  ...
  async _acceptUpdate(update) {
    ...
    try {
      ...
      const patchError = new MuyaRustPatchError(error)
  ...
  _revision(value) {
    ...
    if (!Number.isSafeInteger(revision) || revision < 0) {
      throw new TypeError(`Muya Rust returned an invalid revision: ${String(value)}`)
    }
    return revision
  }
...
export const createMuyaRustBridge = async (factory, markdown, options = {}) => {
  if (typeof factory !== 'function') {
    throw new TypeError('experimentalRustEditor.factory must be a function.')
  }
  const engine = await factory(markdown)
  const bridge = new MuyaRustBridge(engine, options)
