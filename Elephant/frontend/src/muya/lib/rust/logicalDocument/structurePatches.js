...
export const moveNode = (document, nodeId, parentId, index) => {
  ...
  if (node.id === document.root) throw new Error('Muya Rust root node cannot be moved.')
...
export const removeNode = (document, nodeId) => {
  const node = requiredNode(document, nodeId)
  if (node.id === document.root) throw new Error('Muya Rust root node cannot be removed.')
