import { ElephantRustLogicalDocument } from '../logicalDocument'
...
export class ElephantRustDomRenderer {
  constructor(container, options = {}) {
    if (!container || typeof container.replaceChildren !== 'function') {
      throw new TypeError('Elephant Rust DOM renderer requires an Element container.')
    }
    this.container = container
    this.ownerDocument = container.ownerDocument
    this.logical = new ElephantRustLogicalDocument()
  ...
  requiredElement(id) {
    ...
    if (!element) throw new Error(`Elephant Rust DOM element ${String(id)} was not found.`)
    return element
  }

  renderSubtree(id) {
    const node = this.logical.node(id)
    if (!node) throw new Error(`Elephant Rust logical node ${id} is missing.`)
...
export const createDomPatchAdapter = (container, options = {}) => {
  const renderer = new ElephantRustDomRenderer(container, options)
  return {
    renderer,
