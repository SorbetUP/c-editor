...
export const normalizeRevision = (value) => {
  ...
  if (!Number.isSafeInteger(revision) || revision < 0) {
    throw new TypeError(`Invalid Elephant Rust revision: ${String(value)}`)
  }
  return revision
}
...
export const requiredNode = (document, id) => {
  ...
  if (!node) throw new Error(`Elephant Rust node ${id} was not found.`)
  return node
}

export const assertInsertIndex = (parent, index) => {
  if (!Number.isSafeInteger(index) || index < 0 || index > parent.children.length) {
    throw new RangeError(`Invalid child index ${String(index)} for Elephant Rust node ${parent.id}.`)
  }
}
