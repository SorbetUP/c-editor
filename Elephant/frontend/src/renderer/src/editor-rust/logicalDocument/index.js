...
export class ElephantRustLogicalDocument {
  ...
  applyPatches(patches, update) {
    if (!Array.isArray(patches)) throw new TypeError('Elephant Rust patches must be an array.')
    const revision = normalizeRevision(update?.revision)
    const expectedRevision = this.revision + (patches.length > 0 ? 1 : 0)
    if (revision !== expectedRevision) {
      throw new Error(
        `Elephant Rust logical revision mismatch: expected ${expectedRevision}, received ${revision}.`
      )
    }
  ...
  clone() {
    const next = new ElephantRustLogicalDocument()
...
export const createLogicalPatchAdapter = (options = {}) => {
  const document = new ElephantRustLogicalDocument()
  return {
    document,
