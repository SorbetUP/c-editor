...
static char* g_backup_extension_owned = NULL;
static char* g_default_extension_owned = NULL;
...
static char* duplicate_or_default(const char* value, const char* fallback) {
    const char* source = value ? value : fallback;
...
FILE_API FileResult file_create_backup(const char* path) {
    ...
    size_t backup_len = strlen(path) + strlen(g_config.backup_extension) + 1;
    char* backup_path = malloc(backup_len);
    if (!backup_path) {
        set_last_error(FILE_ERROR_OUT_OF_MEMORY);
        return FILE_ERROR_OUT_OF_MEMORY;
    }
    snprintf(backup_path, backup_len, "%s%s", path, g_config.backup_extension);
...
FILE_API void file_manager_set_config(const FileManagerConfig* config) {
    if (config) {
        g_config.max_file_size = config->max_file_size;
        g_config.max_recent_files = config->max_recent_files;
        g_config.auto_backup = config->auto_backup;
        g_config.create_missing_dirs = config->create_missing_dirs;

        char* backup_extension =
            duplicate_or_default(config->backup_extension, ".bak");
        char* default_extension =
            duplicate_or_default(config->default_extension, ".md");

        if (backup_extension) {
            free(g_backup_extension_owned);
            g_backup_extension_owned = backup_extension;
            g_config.backup_extension = g_backup_extension_owned;
        }
        if (default_extension) {
            free(g_default_extension_owned);
            g_default_extension_owned = default_extension;
            g_config.default_extension = g_default_extension_owned;
        }
    }
}
...
FILE_API void file_manager_cleanup(void) {
    free(g_backup_extension_owned);
    free(g_default_extension_owned);
    g_backup_extension_owned = NULL;
    g_default_extension_owned = NULL;
    g_config.backup_extension = ".bak";
    g_config.default_extension = ".md";
}
...
FILE_API char* file_get_documents_dir(void) {
    ...
    if (pw) {
        ...
        if (!docs_path) return NULL;
        snprintf(docs_path, strlen(pw->pw_dir) + 20, "%s/Documents", pw->pw_dir);
        return docs_path;
    }
#endif
...
FILE_API char* file_get_desktop_dir(void) {
    ...
    if (pw) {
        ...
        snprintf(desktop_path, strlen(pw->pw_dir) + 20, "%s/Desktop", pw->pw_dir);
        return desktop_path;
    }
#endif
...
FILE_API char* file_get_backup_path(const char* original_path, const char* timestamp) {
    ...
    size_t timestamp_len = timestamp ? strlen(timestamp) : 0;
    size_t path_len = strlen(dir) + strlen(basename) + timestamp_len + strlen(ext) + 20;
