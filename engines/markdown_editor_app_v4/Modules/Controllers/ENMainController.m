...
- (void)setupTabs {
    ...
    // 2. Files - Navigation et gestion
    // 3. Editor - Création et édition  
    // 4. Search - Recherche dans le contenu
    // 5. Tools - Outils et aide
    ...
    ENFilesTab* filesTab = [[ENFilesTab alloc] init];
    [self registerTab:filesTab forIconType:UI_ICON_FILES];
    [filesTab release];
    ...
    ENToolsTab* toolsTab = [[ENToolsTab alloc] init];
    [self registerTab:toolsTab forIconType:UI_ICON_TOOLS];
    [toolsTab release];
    ...
    NSLog(@"✅ [ENMainController] Onglets configurés : Dashboard → Files → Editor → Search → Tools → Settings");
}
