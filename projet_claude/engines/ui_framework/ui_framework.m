...
bool ui_framework_setup_layout(UIFramework* framework) {
    ...
    // Ajouter les icônes par défaut dans l'ordre spécifié
    ui_framework_add_icon(framework, UI_ICON_SEARCH);   // Loupe
    // UI_ICON_FOLDER sera ajouté plus tard selon tes besoins
    ui_framework_add_icon(framework, UI_ICON_BACK);     // Flèche
    ui_framework_add_icon(framework, UI_ICON_HOME);     // Maison
    ui_framework_add_icon(framework, UI_ICON_SETTINGS); // Roue crantée
...
void ui_framework_add_icon(UIFramework* framework, UIIconType iconType) {
    ...
    NSInteger iconCount = [framework->iconButtons count];
    CGFloat xPosition = (sidebarWidth - iconSize) / 2; // Centrer horizontalement
    CGFloat yPosition;
    
    // Disposition spéciale : settings toujours en bas, autres icônes réparties uniformément
    if (iconType == UI_ICON_SETTINGS) {
        // Settings toujours en bas avec marge de 20px
        yPosition = 20;
    } else {
        // Les autres icônes (search, back, home) sont réparties dans la partie haute
        // Zone disponible pour les icônes normales (en haut) : tout sauf les 80px du bas réservés aux settings
        CGFloat availableHeight = sidebarHeight - 100; // 100px réservés en bas pour settings + marges
        CGFloat topMargin = 40; // Marge du haut
        
        // Compter les icônes non-settings déjà ajoutées
        NSInteger normalIconCount = 0;
        ...
        // Position pour cette icône normale (index = normalIconCount car elle va être ajoutée)
        CGFloat spacingBetween = availableHeight / 4; // Diviser l'espace par 4 pour 3 icônes + marges
        yPosition = sidebarHeight - topMargin - (normalIconCount + 1) * spacingBetween;
    }
    
    NSRect iconFrame = NSMakeRect(xPosition, yPosition, iconSize, iconSize);
...
NSString* ui_framework_get_icon_name(UIIconType iconType) {
    switch (iconType) {
        ...
        case UI_ICON_FOLDER: return @"folder";
...
NSString* ui_framework_get_icon_tooltip(UIIconType iconType) {
    switch (iconType) {
        ...
        case UI_ICON_FOLDER: return @"Dossiers";
        default: return @"";
    }
}
