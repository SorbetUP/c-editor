...
static char* get_app_config_dir(void) {
    ...
    sprintf(config_dir, "%s/%s", home, APP_CONFIG_DIR);
...
static char* get_vault_registry_path(void) {
    ...
    sprintf(registry_path, "%s/%s", config_dir, VAULT_REGISTRY_FILE);
...
VaultResult vault_manager_init(void) {
    ...
    if (home) {
        ...
        sprintf(g_config.default_vault_location, "%s/Documents/Notes", home);
    }
...
VaultResult vault_mark_first_launch_complete(void) {
    ...
    VaultRegistry* registry = malloc(sizeof(VaultRegistry));
...
VaultResult vault_load(const char* vault_path, VaultInfo** info) {
    ...
    *info = malloc(sizeof(VaultInfo));
    memset(*info, 0, sizeof(VaultInfo));
...
VaultResult vault_registry_save(VaultRegistry* registry) {
    printf("🔧 DEBUG vault_registry_save: registry=%p\n", registry);
    if (!registry) {
        printf("❌ DEBUG vault_registry_save: registry NULL\n");
        return VAULT_ERROR_INVALID_PATH;
    }
    
    printf("🔧 DEBUG vault_registry_save: count=%d, default=%s\n", 
           registry->count, 
           registry->default_vault_path ? registry->default_vault_path : "NULL");
    
    char* registry_path = get_vault_registry_path();
    printf("🔧 DEBUG registry_path: %s\n", registry_path ? registry_path : "NULL");
    if (!registry_path) {
        printf("❌ DEBUG vault_registry_save: registry_path NULL\n");
    ...
    printf("🔧 DEBUG JSON à sauvegarder: %s\n", json_string);
    
    fprintf(file, "%s\n", json_string);
    fclose(file);
    json_object_put(root);
    
    printf("✅ DEBUG vault_registry_save terminé avec succès\n");
    return VAULT_SUCCESS;
}

VaultResult vault_registry_add(VaultRegistry* registry, const char* vault_path) {
    printf("🔧 DEBUG vault_registry_add: registry=%p, path=%s\n", registry, vault_path ? vault_path : "NULL");
    
    if (!registry || !vault_path) {
        printf("❌ DEBUG vault_registry_add: paramètre invalide\n");
        return VAULT_ERROR_INVALID_PATH;
    }
    ...
    for (int i = 0; i < registry->count; i++) {
        if (strcmp(registry->vaults[i].config.path, vault_path) == 0) {
            printf("✅ DEBUG vault_registry_add: vault déjà présent\n");
    ...
    printf("🔧 DEBUG vault_load result: %d, info=%p\n", result, info);
    if (result != VAULT_SUCCESS) {
        printf("❌ DEBUG vault_registry_add: échec vault_load\n");
        return result;
    }
    ...
    if (registry->count >= registry->capacity) {
        ...
        registry->vaults = realloc(registry->vaults, sizeof(VaultInfo) * registry->capacity);
        printf("🔧 DEBUG agrandissement tableau: nouvelle capacité=%d\n", registry->capacity);
    }
    
    // Ajouter le vault
    registry->vaults[registry->count] = *info;
    registry->count++;
    printf("✅ DEBUG vault ajouté: count=%d\n", registry->count);
    ...
    if (registry->count == 1 && !registry->default_vault_path) {
        ...
        printf("✅ DEBUG vault défini par défaut: %s\n", vault_path);
    }
    
    free(info);
    printf("✅ DEBUG vault_registry_add terminé avec succès\n");
    return VAULT_SUCCESS;
}
