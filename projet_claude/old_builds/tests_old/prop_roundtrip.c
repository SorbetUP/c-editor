...
// Property: md -> json -> md -> json est idempotent ; pas de marqueurs bruts
// dans les spans ; pas de crash. Compile & link with your lib:
...
static unsigned rndu(void) { // xorshift32
  ...
  x ^= x >> 17;
  x ^= x << 5;
  return rng_state = x ? x : 0xDEADBEEF;
}
static int rndi(int a, int b) {
  return a + (int)(rndu() % (unsigned)(b - a + 1));
}

// petits morceaux réutilisables
static const char *WORDS[] = {"Bonjour", "monde",    "C",     "Markdown",
                              "éditeur", "portable", "UTF-8", "table",
                              "image",   "titre"};
static const char *URLS[] = {"https://ex/x.png", "https://ex/y.jpg",
                             "https://ex/z.svg"};
...
static void gen_image(char **s, size_t *cap, size_t *len) {
  ...
  sb_append(s, cap, len, "![alt](");
  sb_append(s, cap, len, url);
  sb_append(s, cap, len, ")");
  if (rndi(0, 1)) {
    char attrs[64];
    snprintf(attrs, sizeof(attrs), "{w=%d h=%d a=0.%d align=%s}", rndi(32, 320),
             rndi(24, 240), rndi(5, 9),
             (const char *[]){"left", "center", "right"}[rndi(0, 2)]);
...
static void gen_table(char **s, size_t *cap, size_t *len) {
  ...
  for (int c = 0; c < cols; c++) {
    sb_append(s, cap, len, "| ");
    sb_append(s, cap, len, WORDS[rndi(0, 9)]);
    sb_append(s, cap, len, " ");
  }
  sb_append(s, cap, len, "|\n");
  // separator
...
static char *gen_document_md(unsigned seed) {
  ...
  for (int i = 0; i < blocks; i++) {
    ...
    if (kind <= 2) { // paragraphe
      ...
      gen_image(&out, &cap, &len);
      sb_append_ch(&out, &cap, &len, '\n');
    } else {
      gen_table(&out, &cap, &len);
    }
    if (rndi(0, 2) == 0)
      sb_append_ch(&out, &cap, &len, '\n'); // blank line
  }
...
int main(int argc, char **argv) {
  unsigned seed =
      (argc > 1) ? (unsigned)strtoul(argv[1], NULL, 10) : (unsigned)time(NULL);
  ...
  for (int i = 0; i < iters; i++) {
    ...
    Document d0 = {0}, d1 = {0};
    int r = markdown_to_json(md0, &d0);
    assert(r == 0);
    assert_no_raw_markers(&d0);

    char *md1 = NULL;
    r = json_to_markdown(&d0, &md1);
    assert(r == 0 && md1);

    // idempotence au 2nd passage
    r = markdown_to_json(md1, &d1);
    assert(r == 0);
    char *md2 = NULL;
    r = json_to_markdown(&d1, &md2);
    assert(r == 0 && md2);

    // On tolère des diffs cosmétiques ; on exige md1 == md2 (idempotence dès la
    // 1re normalisation)
    if (strcmp(md1, md2) != 0) {
      printf("ERROR: Non-idempotent at iter %d\n", i);
      printf("md1: '%s'\n", md1);
      printf("md2: '%s'\n", md2);
    ...
    r = json_stringify(&d0, &j1);
    assert(r == 0 && j1);
    r = json_stringify(&d1, &j2);
    assert(r == 0 && j2);
    if (strcmp(j1, j2) != 0) {
      printf("ERROR: JSON not identical at iter %d\n", i);
      printf("j1: '%s'\n", j1);
      printf("j2: '%s'\n", j2);
      assert(0 && "non-identical JSON serialization");
    }
