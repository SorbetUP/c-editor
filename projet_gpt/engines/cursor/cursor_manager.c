...
static bool is_position_in_marker_token(const formatting_context_t* context,
                                        int position) {
    if (!context) return false;
    if (context->type == MARKER_NONE) return false;

    if (context->type == MARKER_HEADER) {
        return position >= context->start_pos && position < context->end_pos;
    }

    int opening_end = context->start_pos + context->marker_length;
    int closing_start = context->end_pos - context->marker_length + 1;
    return (position >= context->start_pos && position < opening_end) ||
           (position >= closing_start && position <= context->end_pos);
}

static bool should_add_merge_space(const char* line1, const char* line2,
                                   bool add_space) {
    if (!add_space || !line1 || !line2) return false;

    size_t len1 = strlen(line1);
    size_t len2 = strlen(line2);
    if (len1 == 0 || len2 == 0) return false;
    if (line1[len1 - 1] == ' ' || line2[0] == ' ') return false;

    if (cursor_is_word_char(line1[len1 - 1]) &&
        cursor_is_word_char(line2[0])) {
        return false;
    }
...
cursor_position_t cursor_adjust_for_formatting(int position, const char* content, bool is_markdown_mode) {
    ...
    if (is_position_in_marker_token(&context, position)) {
        if (context.type == MARKER_HEADER ||
            position < context.start_pos + context.marker_length) {
            result.position = context.start_pos;
        } else {
            result.position = context.end_pos + 1;
        }
        CURSOR_DEBUG("Position %d intersects marker token for type %d, adjusting to %d",
                    position, context.type, result.position);
    } else if (context.inside_marker) {
        CURSOR_DEBUG("Position %d is inside formatted content for type %d, keeping split position",
...
cursor_operation_result_t cursor_merge_lines(const char* line1, const char* line2, bool add_space) {
    ...
    bool needs_space = should_add_merge_space(line1, line2, add_space);
