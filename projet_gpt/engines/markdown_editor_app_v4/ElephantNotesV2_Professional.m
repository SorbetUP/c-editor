...
- (void)initializeProfessionalFeatures {
    NSLog(@"🔄 Initializing Professional Features...");
    
    // Initialize vault manager first
    VaultResult initResult = vault_manager_init();
    NSLog(@"📁 Vault manager init result: %d", initResult);
    
    // Check if this is first launch and show setup if needed
    bool isFirstLaunch = vault_is_first_launch();
    NSLog(@"🚀 First launch check: %s", isFirstLaunch ? "YES" : "NO");
    
    if (isFirstLaunch) {
        _isFirstLaunch = YES;
        NSLog(@"🆕 First launch detected - showing vault setup");
        [self showVaultSetup];
        return; // Setup will continue initialization after vault creation
    }
    
    // Load existing vault configuration
    NSLog(@"📂 Loading existing vault configuration...");
...
- (void)loadVaultConfiguration {
    ...
    if (result == VAULT_SUCCESS && registry && registry->default_vault_path) {
        ...
        VaultInfo* info = NULL;
        VaultResult loadResult = vault_load([_vaultPath UTF8String], &info);
        NSLog(@"📂 Vault load result: %d", loadResult);
        if (loadResult == VAULT_SUCCESS && info) {
            NSString* vaultName = [NSString stringWithUTF8String:info->config.name];
            NSLog(@"🏷️  Vault name: %@", vaultName);
        ...
        NSLog(@"⚠️  No vault configured - showing setup");
...
- (void)showVaultManager {
    ...
    vaultManager.vaultChangedHandler = ^(NSString* newVaultPath) {
        [selfRef switchToVault:newVaultPath];
    };
...
- (void)switchToVault:(NSString*)vaultPath {
    if (!vaultPath || [vaultPath isEqualToString:_vaultPath]) return;
    
    // Sauvegarder la session actuelle
    [self saveWorkspaceSession];
    
    // Fermer les timers actuels
    if (_autoSaveTimer) {
        [_autoSaveTimer invalidate];
        _autoSaveTimer = nil;
    }
    ...
    // Configurer le nouveau vault
    _vaultPath = vaultPath;
    _workspacePath = [_vaultPath stringByAppendingPathComponent:@".workspace"];
    
    // Créer le workspace pour le nouveau vault
    NSFileManager* fileManager = [NSFileManager defaultManager];
    if (![fileManager fileExistsAtPath:_workspacePath]) {
        [fileManager createDirectoryAtPath:_workspacePath withIntermediateDirectories:YES attributes:nil error:nil];
    }
    
    // Initialiser la nouvelle session
    _workspace = session_create_workspace([_workspacePath UTF8String]);
    ...
    // Redémarrer les fonctionnalités professionnelles
    [self enableAutoSave];
    _conflictCheckTimer = [NSTimer scheduledTimerWithTimeInterval:10.0 
                                                           target:self 
                                                         selector:@selector(checkForConflicts) 
    ...
    // Nouveau document dans le nouveau vault
    [self newDocument];
    
    NSLog(@"🔄 Switched to vault: %@", _vaultPath);
}
...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        NSMenuItem *vaultManagerItem = [[NSMenuItem alloc] initWithTitle:@"Manage Vaults..." action:@selector(showVaultManager) keyEquivalent:@"v"];
        NSMenuItem *newVaultItem = [[NSMenuItem alloc] initWithTitle:@"New Vault..." action:@selector(showVaultSetup) keyEquivalent:@"V"];
        
        [vaultMenu addItem:vaultManagerItem];
        [vaultMenu addItem:newVaultItem];
        ...
        [vaultManagerItem setTarget:textView];
        [newVaultItem setTarget:textView];
