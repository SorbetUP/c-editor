...
- (instancetype)init {
    ...
    if (self) {
        ...
        [[AppLogger sharedLogger] initializeWithAppPath:appPath];
        [[AppSettings sharedSettings] initializeWithAppPath:appPath];
        
        LogMessageInfo(LOG_SECTION_APP, @"🔧 Initialisation du contrôleur ElephantNotes V3");
        LogMessageDebug(LOG_SECTION_APP, @"📁 Chemin de l'app: %@", appPath);
...
- (void)setupApplication {
    ...
    // Synchroniser AppSettings avec le vault registry
    [self synchronizeAppSettingsWithVaultRegistry];
...
- (void)checkFirstLaunch {
    ...
    // CORRECTION: Si on a un vault par défaut valide, on ne force pas le premier lancement
    _isFirstLaunch = isFirstLaunch || !hasDefaultVault;
...
- (void)showSearchMode {
    LogMessageInfo(LOG_SECTION_SEARCH, @"🔍 Mode recherche activé");
    NSLog(@"🔍 Mode recherche activé");
    
    if (!_uiFramework) {
        LogMessageError(LOG_SECTION_SEARCH, @"❌ UI Framework non initialisé");
        NSLog(@"❌ UI Framework non initialisé");
        return;
    }
    
    // Masquer l'interface de recherche si elle est visible
    [self hideSearchInterface];
    
    // Générer l'interface de recherche comme contenu markdown
    ...
    LogMessageInfo(LOG_SECTION_SEARCH, @"✅ Mode recherche activé avec succès");
}

- (void)createModernSearchInterface {
    // Méthode supprimée - remplacée par generateSearchInterface()
    NSLog(@"⚠️ createModernSearchInterface supprimée - utilisez generateSearchInterface");
    return;
}

- (void)loadFilesForModernSearch {
    if (!_currentVaultPath) {
        LogMessageWarning(LOG_SECTION_SEARCH, @"⚠️ Aucun vault configuré pour la recherche");
        return;
    }
    
    LogMessageInfo(LOG_SECTION_SEARCH, @"📁 Scan optimisé des fichiers depuis: %@", _currentVaultPath);
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSFileManager* fileManager = [NSFileManager defaultManager];
        NSMutableArray* fileData = [[NSMutableArray alloc] init];
        
        // Scan récursif optimisé
        [self scanDirectoryRecursively:self->_currentVaultPath fileManager:fileManager results:fileData];
        
        // Trier par nom
        [fileData sortUsingComparator:^NSComparisonResult(NSDictionary* a, NSDictionary* b) {
            return [a[@"name"] localizedCaseInsensitiveCompare:b[@"name"]];
        }];
        
        dispatch_async(dispatch_get_main_queue(), ^{
            self->_searchFileList = fileData;
            self->_filteredFileList = [fileData mutableCopy];
            [self->_searchTableView reloadData];
            [self updateResultsLabel];
            
            LogMessageInfo(LOG_SECTION_SEARCH, @"✅ %ld fichiers indexés", [fileData count]);
        });
    });
}

- (void)scanDirectoryRecursively:(NSString*)path fileManager:(NSFileManager*)fileManager results:(NSMutableArray*)results {
    NSError* error = nil;
    NSArray* contents = [fileManager contentsOfDirectoryAtPath:path error:&error];
    if (error) return;
    
    for (NSString* item in contents) {
        if ([item hasPrefix:@"."]) continue; // Ignorer les fichiers cachés
        
        NSString* fullPath = [path stringByAppendingPathComponent:item];
        NSString* relativePath = [fullPath stringByReplacingOccurrencesOfString:_currentVaultPath withString:@""];
        ...
        if (isDirectory) {
            ...
            [self scanDirectoryRecursively:fullPath fileManager:fileManager results:results];
        } else if ([[item pathExtension] isEqualToString:@"md"]) {
            // Ajouter le fichier markdown avec métadonnées
            ...
            NSDictionary* fileInfo = @{
                ...
                @"size": fileSize ?: @0,
                @"modified": modificationDate ?: [NSDate distantPast],
                @"icon": @"📝"
            };
...
- (void)performLiveSearch:(NSTextField*)sender {
    NSString* query = [sender stringValue];
    LogMessageInfo(LOG_SECTION_SEARCH, @"🔍 Recherche en temps réel: '%@'", query);
    ...
    // Recherche instantanée pour les requêtes courtes, délai pour les longues
    NSTimeInterval delay = ([query length] <= 2) ? 0.1 : 0.2;
    
    _searchTimer = [NSTimer scheduledTimerWithTimeInterval:delay
                                                   target:self
...
- (void)executeSearch:(NSTimer*)timer {
    NSString* query = timer.userInfo[@"query"];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if ([query length] == 0) {
            ...
            filtered = [self->_searchFileList mutableCopy];
        } else {
            NSString* lowerQuery = [query lowercaseString];
            
            for (NSDictionary* fileInfo in self->_searchFileList) {
                ...
                if ([[fileName lowercaseString] containsString:lowerQuery] ||
                    [[relativePath lowercaseString] containsString:lowerQuery]) {
                    [filtered addObject:fileInfo];
                }
            }
        }
        
        dispatch_async(dispatch_get_main_queue(), ^{
            self->_filteredFileList = filtered;
            [self->_searchTableView reloadData];
            [self updateResultsLabel];
            
            LogMessageInfo(LOG_SECTION_SEARCH, @"✅ %ld fichiers trouvés pour '%@'", [filtered count], query);
        });
    });
}

- (void)clearSearch:(NSButton*)sender {
    [_searchField setStringValue:@""];
    [self performLiveSearch:_searchField];
    [_searchField becomeFirstResponder];
}

- (void)updateResultsLabel {
    NSUInteger count = [_filteredFileList count];
    NSString* text;
    ...
    if (count == 0) {
        ...
        text = [NSString stringWithFormat:@"%ld fichiers", count];
    }
    
    [_resultsLabel setStringValue:text];
}

- (void)openSelectedFile:(NSTableView*)tableView {
    NSInteger selectedRow = [tableView selectedRow];
    if (selectedRow >= 0 && selectedRow < [_filteredFileList count]) {
        NSDictionary* fileInfo = _filteredFileList[selectedRow];
        NSString* filePath = fileInfo[@"path"];
        
        LogMessageInfo(LOG_SECTION_SEARCH, @"📄 Ouverture du fichier: %@", fileInfo[@"name"]);
        [self loadFileContent:filePath];
    }
}

// MARK: - NSTableView Data Source Methods

- (NSInteger)numberOfRowsInTableView:(NSTableView *)tableView {
    return [_filteredFileList count];
}

- (id)tableView:(NSTableView *)tableView objectValueForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
    if (row >= [_filteredFileList count]) return nil;
    
    NSDictionary* fileInfo = _filteredFileList[row];
    return fileInfo; // Retourner l'objet complet pour le delegate
}

// MARK: - NSTableView Delegate Methods

- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
    static NSString* cellIdentifier = @"ModernFileCell";
    
    // Utiliser le cache de cellules pour les performances
    NSTableCellView* cellView = [tableView makeViewWithIdentifier:cellIdentifier owner:self];
    if (!cellView) {
        // Créer une nouvelle cellule seulement si nécessaire
        cellView = [[NSTableCellView alloc] init];
        cellView.identifier = cellIdentifier;
        
        // Créer le layout de la cellule moderne
        NSTextField* titleField = [[NSTextField alloc] init];
        [titleField setTranslatesAutoresizingMaskIntoConstraints:NO];
        [titleField setBezeled:NO];
        [titleField setDrawsBackground:NO];
        [titleField setEditable:NO];
        [titleField setSelectable:NO];
        [titleField setFont:[NSFont systemFontOfSize:13 weight:NSFontWeightMedium]];
        [cellView addSubview:titleField];
        cellView.textField = titleField;
        
        NSTextField* subtitleField = [[NSTextField alloc] init];
        [subtitleField setTranslatesAutoresizingMaskIntoConstraints:NO];
        ...
        // Tag pour récupérer le subtitle
        [subtitleField setTag:100];
        
        // Contraintes
        [NSLayoutConstraint activateConstraints:@[
            [titleField.topAnchor constraintEqualToAnchor:cellView.topAnchor constant:4],
            [titleField.leadingAnchor constraintEqualToAnchor:cellView.leadingAnchor constant:8],
            [titleField.trailingAnchor constraintEqualToAnchor:cellView.trailingAnchor constant:-8],
            
            [subtitleField.topAnchor constraintEqualToAnchor:titleField.bottomAnchor constant:2],
            [subtitleField.leadingAnchor constraintEqualToAnchor:cellView.leadingAnchor constant:8],
            [subtitleField.trailingAnchor constraintEqualToAnchor:cellView.trailingAnchor constant:-8],
            [subtitleField.bottomAnchor constraintEqualToAnchor:cellView.bottomAnchor constant:-4]
        ]];
    }
    
    // Remplir les données
    if (row < [_filteredFileList count]) {
        NSDictionary* fileInfo = _filteredFileList[row];
        NSString* fileName = fileInfo[@"name"];
        NSString* relativePath = fileInfo[@"relativePath"];
        NSDate* modifiedDate = fileInfo[@"modified"];
        
        // Titre avec icône
        cellView.textField.stringValue = [NSString stringWithFormat:@"📝 %@", fileName];
        
        // Sous-titre avec chemin et date
        NSDateFormatter* formatter = [[NSDateFormatter alloc] init];
        [formatter setDateStyle:NSDateFormatterShortStyle];
        [formatter setTimeStyle:NSDateFormatterShortStyle];
        NSString* dateString = [formatter stringFromDate:modifiedDate];
        
        NSTextField* subtitleField = [cellView viewWithTag:100];
        subtitleField.stringValue = [NSString stringWithFormat:@"%@ • %@", relativePath, dateString];
    }
...
- (CGFloat)tableView:(NSTableView *)tableView heightOfRow:(NSInteger)row {
    return 44.0; // Hauteur fixe pour les performances
}
...
- (void)tableViewSelectionDidChange:(NSNotification *)notification {
    ...
    if (selectedRow >= 0 && selectedRow < [_filteredFileList count]) {
        ...
        LogMessageInfo(LOG_SECTION_SEARCH, @"📄 Fichier sélectionné: %@", fileInfo[@"name"]);
        
        // Ouvrir le fichier avec un délai pour permettre l'animation de sélection
...
// Méthode supprimée - remplacée par l'interface simple
...
- (NSString*)generateSearchInterface {
    ...
    // En-tête
    [content appendString:@"# 🔍 Recherche - ElephantNotes V3\n\n"];
    ...
    if (_currentVaultPath) {
        ...
        if (_searchEngine) {
            ...
            [content appendString:@"**Index prêt :** ❌ Non initialisé\n\n"];
        }
    ...
    if (_searchEngine) {
        ...
        [content appendFormat:@"- **Requêtes traitées :** %d\n", stats.total_queries];
        [content appendFormat:@"- **Temps moyen :** %.2f ms\n", stats.avg_query_time_ms];
        [content appendFormat:@"- **Utilisation mémoire :** %zu MB\n", stats.memory_usage_mb];
        [content appendString:@"- **Cache hits :** Optimise\n\n"];
    }
...
- (NSArray*)getAllFilesInVault {
    ...
    NSString* notesPath = [_currentVaultPath stringByAppendingPathComponent:@"Notes"];
...
// MARK: - Interface de recherche visuelle

- (void)createSearchInterface {
    // Méthode supprimée - remplacée par generateSearchInterface()
    NSLog(@"⚠️ createSearchInterface supprimée - utilisez generateSearchInterface");
    return;
}
    
    if (!_uiFramework || !_uiFramework->containerView) {
        LogMessageErrorSub(LOG_SECTION_SEARCH, @"Interface", @"❌ UI Framework non initialisé");
        return;
    }
    
    // Récupérer les dimensions du conteneur et de la barre latérale
    NSRect containerFrame = [_uiFramework->containerView bounds];
    CGFloat sidebarWidth = _uiFramework->sidebarConfig.width; // Largeur de la barre latérale (60px)
    
    // Créer la zone de recherche à droite de la barre latérale
    NSRect searchAreaFrame = NSMakeRect(sidebarWidth, 0, 
                                       containerFrame.size.width - sidebarWidth, 
                                       containerFrame.size.height);
    
    _searchView = [[NSView alloc] initWithFrame:searchAreaFrame];
    [_searchView setHidden:YES];
    
    // Configuration de la barre de recherche (en haut)
    CGFloat searchBarHeight = 40;
    CGFloat padding = 10;
    NSRect searchFrame = NSMakeRect(padding, searchAreaFrame.size.height - searchBarHeight - padding, 
                                  searchAreaFrame.size.width - 2*padding, searchBarHeight);
    
    _searchField = [[NSTextField alloc] initWithFrame:searchFrame];
    [_searchField setPlaceholderString:@"Rechercher dans les notes..."];
    [_searchField setTarget:self];
    [_searchField setAction:@selector(handleSearchFieldChange:)];
    
    // Configuration de l'arborescence (en bas)
    NSRect treeFrame = NSMakeRect(padding, padding, 
                                 searchAreaFrame.size.width - 2*padding, 
                                 searchAreaFrame.size.height - searchBarHeight - 3*padding);
    
    _treeScrollView = [[NSScrollView alloc] initWithFrame:treeFrame];
    [_treeScrollView setHasVerticalScroller:YES];
    [_treeScrollView setHasHorizontalScroller:YES];
    [_treeScrollView setAutohidesScrollers:NO];
    
    _fileTreeView = [[NSOutlineView alloc] init];
    [_fileTreeView setHeaderView:nil];
    [_fileTreeView setTarget:self];
    [_fileTreeView setAction:@selector(handleFileSelection:)];
    [_fileTreeView setDataSource:self];
    [_fileTreeView setDelegate:self];
    
    // Ajouter une colonne pour les noms de fichiers
    NSTableColumn *column = [[NSTableColumn alloc] initWithIdentifier:@"fileName"];
    [column setTitle:@"Fichiers"];
    [column setWidth:treeFrame.size.width - 20];
    [_fileTreeView addTableColumn:column];
    [column release];
    
    [_treeScrollView setDocumentView:_fileTreeView];
    
    // Ajouter les vues à la vue de recherche
    [_searchView addSubview:_searchField];
    [_searchView addSubview:_treeScrollView];
    
    // Initialiser les données de l'arborescence
    _fileTreeData = [[NSMutableArray alloc] init];
    
    LogMessageInfoSub(LOG_SECTION_SEARCH, @"Interface", @"✅ Interface de recherche créée");
}

- (void)showSearchInterface {
    // Méthode supprimée - remplacée par generateSearchInterface()
    NSLog(@"⚠️ showSearchInterface supprimée - utilisez generateSearchInterface");
    return;
}
    
    if (!_searchView || !_uiFramework || !_uiFramework->containerView) {
        NSLog(@"❌ Interface de recherche non initialisée");
        return;
    }
    
    // Cacher l'éditeur principal
    if (_uiFramework->editorScrollView) {
        [_uiFramework->editorScrollView setHidden:YES];
    }
    
    // Ajouter et afficher la vue de recherche dans le conteneur principal
    [_uiFramework->containerView addSubview:_searchView];
    [_searchView setHidden:NO];
    _searchViewVisible = YES;
    
    // Charger l'arborescence des fichiers
    [self loadFileTree];
    
    NSLog(@"✅ Interface de recherche affichée");
}

- (void)hideSearchInterface {
    NSLog(@"🔍 Masquage de l'interface de recherche");
    
    if (!_searchView) {
        return;
    }
    
    // Masquer la vue de recherche
    [_searchView setHidden:YES];
    [_searchView removeFromSuperview];
    _searchViewVisible = NO;
    
    // Réafficher l'éditeur principal
    if (_uiFramework->editorScrollView) {
        [_uiFramework->editorScrollView setHidden:NO];
    }
    
    NSLog(@"✅ Interface de recherche masquée");
}

- (void)focusSearchField {
    NSLog(@"🎯 Focus sur le champ de recherche");
    
    if (_searchField && _searchViewVisible) {
        [[_searchField window] makeFirstResponder:_searchField];
        NSLog(@"✅ Focus défini sur le champ de recherche");
    }
}

- (void)loadFileTree {
    NSLog(@"📁 Chargement de l'arborescence des fichiers");
    
    if (!_currentVaultPath || !_fileTreeData) {
        NSLog(@"❌ Vault ou données d'arborescence non initialisés");
        return;
    }
    
    [_fileTreeData removeAllObjects];
    [self loadDirectoryContents:_currentVaultPath intoArray:_fileTreeData];
    
    // Recharger la vue
    [_fileTreeView reloadData];
    
    NSLog(@"✅ Arborescence chargée avec %lu éléments", [_fileTreeData count]);
}

- (void)loadDirectoryContents:(NSString*)directoryPath intoArray:(NSMutableArray*)parentArray {
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSError *error = nil;
    NSArray *contents = [fileManager contentsOfDirectoryAtPath:directoryPath error:&error];
    
    if (error) {
        NSLog(@"❌ Erreur lors du chargement du dossier %@: %@", directoryPath, [error localizedDescription]);
        return;
    }
    
    // Trier les contenus : dossiers d'abord, puis fichiers
    NSArray *sortedContents = [contents sortedArrayUsingComparator:^NSComparisonResult(NSString *obj1, NSString *obj2) {
        NSString *fullPath1 = [directoryPath stringByAppendingPathComponent:obj1];
        NSString *fullPath2 = [directoryPath stringByAppendingPathComponent:obj2];
        
        BOOL isDir1, isDir2;
        [fileManager fileExistsAtPath:fullPath1 isDirectory:&isDir1];
        [fileManager fileExistsAtPath:fullPath2 isDirectory:&isDir2];
        
        // Masquer les fichiers cachés (commençant par un point)
        if ([obj1 hasPrefix:@"."]) return NSOrderedDescending;
        if ([obj2 hasPrefix:@"."]) return NSOrderedAscending;
    ...
    // Ajouter les fichiers et dossiers à l'arborescence
    for (NSString *item in sortedContents) {
        ...
        if ([fileManager fileExistsAtPath:fullPath isDirectory:&isDirectory]) {
            ...
            [node setObject:item forKey:@"name"];
            [node setObject:fullPath forKey:@"path"];
            [node setObject:[NSNumber numberWithBool:isDirectory] forKey:@"isDirectory"];
            [node setObject:[NSNumber numberWithBool:NO] forKey:@"isExpanded"];
            
            if (isDirectory) {
                NSMutableArray *children = [NSMutableArray array];
                [node setObject:children forKey:@"children"];
                // Pré-charger les enfants pour permettre le dépliage
                [self loadDirectoryContents:fullPath intoArray:children];
            }
            
            [parentArray addObject:node];
        }
    }
}
...
- (void)handleSearchFieldChange:(NSTextField*)sender {
    NSString *query = [sender stringValue];
    NSLog(@"🔍 Recherche: '%@'", query);
    
    if ([query length] > 0) {
        [self performSearchInTree:query];
    } else {
...
- (void)performSearchInTree:(NSString*)query {
    NSLog(@"🔍 Recherche dans l'arborescence: '%@'", query);
    
    // Pour l'instant, utilisons seulement la recherche simple par nom de fichier
    [self simpleFileNameSearch:query];
}

- (void)simpleFileNameSearch:(NSString*)query {
    NSLog(@"🔍 Recherche simple dans les noms de fichiers: '%@'", query);
    
    NSMutableArray *filteredData = [NSMutableArray array];
    [self filterTreeData:_fileTreeData withQuery:query intoArray:filteredData];
    
    // Afficher seulement les résultats filtrés
    NSMutableArray *originalData = _fileTreeData;
    _fileTreeData = filteredData;
    [_fileTreeView reloadData];
}

- (void)filterTreeData:(NSArray*)treeData withQuery:(NSString*)query intoArray:(NSMutableArray*)filteredArray {
    for (NSDictionary *node in treeData) {
        NSString *name = [node objectForKey:@"name"];
        BOOL isDirectory = [[node objectForKey:@"isDirectory"] boolValue];
        
        // Vérifier si le nom contient la requête (insensible à la casse)
        if ([name rangeOfString:query options:NSCaseInsensitiveSearch].location != NSNotFound) {
            [filteredArray addObject:node];
        }
        
        // Si c'est un dossier, chercher aussi dans ses enfants
        if (isDirectory) {
            NSArray *children = [node objectForKey:@"children"];
            if (children && [children count] > 0) {
                NSMutableArray *filteredChildren = [NSMutableArray array];
                [self filterTreeData:children withQuery:query intoArray:filteredChildren];
                
                if ([filteredChildren count] > 0) {
                    // Si des enfants correspondent, ajouter ce dossier aussi
                    NSMutableDictionary *folderWithResults = [node mutableCopy];
                    [folderWithResults setObject:filteredChildren forKey:@"children"];
                    [folderWithResults setObject:[NSNumber numberWithBool:YES] forKey:@"isExpanded"];
                    [filteredArray addObject:folderWithResults];
                    [folderWithResults release];
                }
            }
        }
    }
...
- (void)addMatchingFileToTree:(NSString*)filePath intoArray:(NSMutableArray*)treeArray {
    NSString *fileName = [filePath lastPathComponent];
    NSString *directoryPath = [filePath stringByDeletingLastPathComponent];
    
    // Créer un nœud pour ce fichier
    NSMutableDictionary *fileNode = [NSMutableDictionary dictionary];
    [fileNode setObject:fileName forKey:@"name"];
    [fileNode setObject:filePath forKey:@"path"];
    [fileNode setObject:[NSNumber numberWithBool:NO] forKey:@"isDirectory"];
    [fileNode setObject:[NSNumber numberWithBool:NO] forKey:@"isExpanded"];
    
    [treeArray addObject:fileNode];
}

- (void)handleFileSelection:(NSOutlineView*)outlineView {
    LogMessageDebugSub(LOG_SECTION_SEARCH, @"TreeView", @"📁 Début de handleFileSelection");
    
    @try {
        NSInteger selectedRow = [outlineView selectedRow];
        if (selectedRow < 0) {
            LogMessageWarningSub(LOG_SECTION_SEARCH, @"TreeView", @"❌ Aucune ligne sélectionnée");
            return;
        }
        
        LogMessageDebugSub(LOG_SECTION_SEARCH, @"TreeView", @"Ligne sélectionnée: %ld", selectedRow);
        
        id item = [outlineView itemAtRow:selectedRow];
        if (!item) {
            LogMessageErrorSub(LOG_SECTION_SEARCH, @"TreeView", @"❌ Aucun item à la ligne %ld", selectedRow);
            return;
        }
        
        LogMessageDebugSub(LOG_SECTION_SEARCH, @"TreeView", @"Item récupéré: %@", [item class]);
        
        if (![item isKindOfClass:[NSDictionary class]]) {
            LogMessageErrorSub(LOG_SECTION_SEARCH, @"TreeView", @"❌ Item n'est pas un dictionnaire: %@", item);
            return;
        }
        
        NSDictionary *node = (NSDictionary*)item;
        NSString *filePath = [node objectForKey:@"path"];
        BOOL isDirectory = [[node objectForKey:@"isDirectory"] boolValue];
        
        LogMessageInfoSub(LOG_SECTION_SEARCH, @"TreeView", @"📁 Sélection: %@ (dossier: %@)", filePath, isDirectory ? @"OUI" : @"NON");
        
        if (!isDirectory && [filePath hasSuffix:@".md"]) {
            // Ouvrir le fichier markdown
            LogMessageInfoSub(LOG_SECTION_SEARCH, @"TreeView", @"📝 Ouverture du fichier: %@", filePath);
            [self hideSearchInterface];
            [self loadFileContent:filePath];
            [self switchToMode:UI_ICON_HOME]; // Revenir au mode éditeur
        } else if (isDirectory) {
            // Basculer l'expansion du dossier
            LogMessageDebugSub(LOG_SECTION_SEARCH, @"TreeView", @"📁 Basculement de l'expansion du dossier");
            
            if (![item isKindOfClass:[NSMutableDictionary class]]) {
                LogMessageErrorSub(LOG_SECTION_SEARCH, @"TreeView", @"❌ Le noeud n'est pas mutable");
                return;
            }
            
            NSMutableDictionary *mutableNode = (NSMutableDictionary*)node;
            BOOL isExpanded = [[mutableNode objectForKey:@"isExpanded"] boolValue];
            LogMessageDebugSub(LOG_SECTION_SEARCH, @"TreeView", @"📁 État actuel: %@", isExpanded ? @"DÉPLIÉ" : @"PLIÉ");
            
            [mutableNode setObject:[NSNumber numberWithBool:!isExpanded] forKey:@"isExpanded"];
            LogMessageDebugSub(LOG_SECTION_SEARCH, @"TreeView", @"📁 Nouvel état: %@", !isExpanded ? @"DÉPLIÉ" : @"PLIÉ");
            
            // Vérifier que _fileTreeView existe avant de recharger
            if (!_fileTreeView) {
                LogMessageErrorSub(LOG_SECTION_SEARCH, @"TreeView", @"❌ _fileTreeView est NULL");
                return;
            }
            
            if (![_fileTreeView superview]) {
                LogMessageErrorSub(LOG_SECTION_SEARCH, @"TreeView", @"❌ _fileTreeView n'a pas de superview");
                return;
            }
            
            LogMessageDebugSub(LOG_SECTION_SEARCH, @"TreeView", @"📁 Rechargement de l'arborescence...");
            
            // Utiliser dispatch_async pour éviter les problèmes de réentrance
            dispatch_async(dispatch_get_main_queue(), ^{
                @try {
                    [_fileTreeView reloadData];
                    LogMessageDebugSub(LOG_SECTION_SEARCH, @"TreeView", @"📁 Rechargement terminé avec succès");
                }
                @catch (NSException *reloadException) {
                    LogMessageErrorSub(LOG_SECTION_SEARCH, @"TreeView", @"❌ Exception lors du reload: %@", reloadException.description);
                    [[AppLogger sharedLogger] logException:reloadException section:LOG_SECTION_SEARCH context:@"reloadData"];
                }
            });
        }
        
        LogMessageDebugSub(LOG_SECTION_SEARCH, @"TreeView", @"📁 Fin de handleFileSelection");
        
    }
    @catch (NSException *exception) {
        LogMessageCritical(LOG_SECTION_SEARCH, @"❌ CRASH dans handleFileSelection: %@", exception.description);
        [[AppLogger sharedLogger] logException:exception section:LOG_SECTION_SEARCH context:@"handleFileSelection"];
    }
}


- (void)synchronizeAppSettingsWithVaultRegistry {
    LogMessageInfo(LOG_SECTION_APP, @"🔄 Synchronisation AppSettings avec VaultRegistry");
    
    AppSettings* settings = [AppSettings sharedSettings];
    
    if (_vaultRegistry && _vaultRegistry->default_vault_path) {
        NSString* defaultVaultPath = [NSString stringWithUTF8String:_vaultRegistry->default_vault_path];
        
        // Mettre à jour les settings avec le vault par défaut
        settings.currentVaultPath = defaultVaultPath;
        settings.defaultVaultPath = defaultVaultPath;
        
        // Ajouter aux vaults récents si pas déjà présent
        [settings addRecentVault:defaultVaultPath];
        
        LogMessageInfo(LOG_SECTION_APP, @"✅ AppSettings synchronisé avec vault: %@", defaultVaultPath);
        
        // Marquer le système de vaults comme prêt
        _vaultSystemReady = true;
        
    } else {
        LogMessageWarning(LOG_SECTION_APP, @"⚠️ Aucun vault par défaut trouvé dans le registry");
        _vaultSystemReady = false;
    }
