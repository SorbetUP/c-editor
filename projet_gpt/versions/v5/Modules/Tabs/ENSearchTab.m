...
- (NSArray*)searchNodesForQuery:(NSString*)query {
    ...
    [self ensureSearchEngineInitialized];
    if (!_searchEngine) {
        return [self fallbackNodesForQuery:query];
    }
...
- (NSArray*)fallbackNodesForQuery:(NSString*)query {
    if (!query || [query length] == 0) {
        return [NSArray array];
    }
    NSString* lowered = [query lowercaseString];
    NSMutableArray* matches = [NSMutableArray array];
    [self collectNodes:_allNodes matching:lowered results:matches];
    return matches;
}

- (void)collectNodes:(NSArray*)nodes matching:(NSString*)lowered results:(NSMutableArray*)results {
    if (!nodes || !lowered)
        return;
    for (ENFileNode* node in nodes) {
        if (!node)
            continue;
        NSString* candidate = node.relativePath ?: node.name;
        if (candidate && [[candidate lowercaseString] containsString:lowered]) {
            [results addObject:node];
        }
        if (node.isDirectory && node.children) {
            [self collectNodes:node.children matching:lowered results:results];
        }
    }
}
