...
static void ensure_elements_capacity(Document *doc, size_t needed) {
    if (doc->elements_capacity < needed) {
        size_t new_capacity = doc->elements_capacity == 0 ? 8 : doc->elements_capacity * 2;
...
static void ensure_line_capacity(Document *doc, size_t needed) {
    if (doc->current_line_capacity < needed) {
        size_t new_capacity = doc->current_line_capacity == 0 ? 64 : doc->current_line_capacity * 2;
        while (new_capacity < needed) new_capacity *= 2;
...
static ElementText create_text_element(Document *doc, const char *text, int level) {
    ElementText elem;
...
void editor_commit_line(Document *doc) {
    ...
    if (is_table_separator_line(doc->current_line)) {
        ...
        if (doc->elements_len > 0 && doc->elements[doc->elements_len - 1].kind == T_TEXT) {
            ElementText *prev_text = &doc->elements[doc->elements_len - 1].as.text;
            
            // Check if previous line could be a table header (has spans indicating pipe content)
            // We need to reconstruct the original text from spans to check for pipes
            ...
            if (has_pipes) {
                ...
                for (int c = 0; c < col_count; c++) {
                    ...
                    table.cells[0][c]->text = strdup_safe(header_cols[c] ? header_cols[c] : "");
    ...
    if (parse_image_line(doc->current_line, &image) == 0) {
        ...
        text_elem.spans = convert_spans_to_text_spans(text_elem.text, span_count > 0 ? spans : NULL, span_count > 0 ? span_count : 0, &text_elem.spans_count);
        free(text_elem.text);
        text_elem.text = NULL;
