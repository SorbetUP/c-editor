...
static int parse_bool_value(JsonParser *parser, bool *out) {
    ...
    if (parser->pos + 4 <= parser->len && strncmp(parser->json + parser->pos, "true", 4) == 0) {
        *out = true;
        parser->pos += 4;
        return 0;
    } else if (parser->pos + 5 <= parser->len && strncmp(parser->json + parser->pos, "false", 5) == 0) {
        *out = false;
        parser->pos += 5;
        return 0;
    }
...
static void write_rgba_array(FILE *fp, const RGBA *rgba) {
    ...
    fprintf(fp, "[%.3f,%.3f,%.3f,%.3f]", normalized.r, normalized.g, normalized.b, normalized.a);
}
...
static int write_element_text(FILE *fp, const ElementText *text) {
    ...
    switch (text->align) {
        ...
        case ALIGN_JUSTIFY: align_str = "justify"; break;
        case ALIGN_LEFT: default: align_str = "left"; break;
    }
...
static int write_element_image(FILE *fp, const ElementImage *image) {
    ...
    switch (image->align) {
        ...
        case ALIGN_RIGHT: align_str = "right"; break;
        case ALIGN_LEFT: default: align_str = "left"; break;
        case ALIGN_JUSTIFY: align_str = "left"; break;
    }
    fprintf(fp, ",\"align\":\"%s\"", align_str);
...
int json_parse(const char *json_str, Document *doc) {
    ...
    if (elements_parser.pos >= elements_parser.len || elements_parser.json[elements_parser.pos] != '[') {
        return -1;
    }
    elements_parser.pos++;
    ...
    while (elements_parser.pos < elements_parser.len) {
        ...
        while (elements_parser.pos < elements_parser.len) {
            ...
            if (c == '{') depth++;
                ...
                elements_parser.pos++;
                while (elements_parser.pos < elements_parser.len && elements_parser.json[elements_parser.pos] != '"') {
                    if (elements_parser.json[elements_parser.pos] == '\\') elements_parser.pos++;
                    elements_parser.pos++;
                }
            }
            elements_parser.pos++;
        ...
        JsonParser elem_parser = { json_str, obj_start, elements_parser.pos - obj_start };
