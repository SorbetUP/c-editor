...
TextSpan *convert_spans_to_text_spans(const char *text, const InlineSpan *spans, size_t span_count, size_t *out_count) {
    ...
    for (size_t i = 0; i < span_count; i++) {
        ...
        result[result_count].italic = (spans[i].style == INLINE_ITALIC || spans[i].style == INLINE_BOLD_ITALIC);
...
int parse_table_block(MarkdownParser *parser, ElementTable *table) {
    ...
    if (!line_end) line_end = parser->text + parser->len;
    ...
    for (size_t r = 0; r < table->rows; r++) {
        ...
        for (size_t c = 0; c < table->cols; c++) {
            ...
            if (c < (size_t)first_row_cols && all_rows[r][c]) {
    ...
    for (size_t r = 0; r < table->rows; r++) {
        for (int c = 0; c < first_row_cols; c++) {
            free(all_rows[r][c]);
        }
        free(all_rows[r]);
    }
...
int markdown_to_json(const char *markdown, Document *doc) {
    ...
    while (pos < end) {
        ...
        if (parse_image_line(line, &image) == 0) {
            ...
            if (span_count > 0) {
                text.spans = convert_spans_to_text_spans(text.text, spans, span_count, &text.spans_count);
            } else {
                text.spans = convert_spans_to_text_spans(text.text, NULL, 0, &text.spans_count);
            }
...
static void write_inline_markup(FILE *fp, const char *text, bool bold, bool italic, bool highlight, bool underline) {
    if (bold && italic) {
        fprintf(fp, "***%s***", text);
    } else if (bold) {
        fprintf(fp, "**%s**", text);
    ...
    for (size_t i = 0; i < doc->elements_len; i++) {
        ...
        switch (elem->kind) {
            ...
            case T_IMAGE: {
                ...
                if (image->width > 0 || image->height > 0 || image->alpha != 1.0f || image->align != ALIGN_LEFT) {
                    fprintf(fp, "{");
                    bool first = true;
                    ...
                    if (image->align != ALIGN_LEFT) {
                        ...
                        switch (image->align) {
                            case ALIGN_CENTER: align_str = "center"; break;
                            case ALIGN_RIGHT: align_str = "right"; break;
            ...
            case T_TABLE: {
                ...
                for (size_t c = 0; c < table->cols; c++) {
                    ...
                    if (table->cells[0] && table->cells[0][c] && table->cells[0][c]->text) {
                        write_inline_markup(fp, table->cells[0][c]->text,
                                          table->cells[0][c]->bold,
                                          table->cells[0][c]->italic,
                                          table->cells[0][c]->has_highlight,
                ...
                for (size_t r = 1; r < table->rows; r++) {
                    for (size_t c = 0; c < table->cols; c++) {
                        ...
                        if (table->cells[r] && table->cells[r][c] && table->cells[r][c]->text) {
                            write_inline_markup(fp, table->cells[r][c]->text,
                                              table->cells[r][c]->bold,
                                              table->cells[r][c]->italic,
                                              table->cells[r][c]->has_highlight,
