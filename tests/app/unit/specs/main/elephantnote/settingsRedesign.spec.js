...
describe('Elephant settings redesign', () => {
  ...
  it('keeps official packages independent from the Community Addons boundary', () => {
    ...
    expect(source).not.toContain('id="en-addons-title-actions"')
    expect(panel).not.toContain('<Teleport defer to="#en-addons-title-actions">')
    expect(panel).not.toContain('en-community-title-check')
    expect(panel).not.toContain('toggleCommunityAddons')
    expect(panel).not.toContain(':aria-checked="communityAddonsEnabled"')
