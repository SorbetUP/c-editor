...
// Fuzz léger (sans lib externe) : génère des séquences adversariales et vérifie
// non-crash + invariants clés.
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static unsigned u(void) {
  S ^= S << 13;
  S ^= S >> 17;
  S ^= S << 5;
  return S ? S : 0xBEEFCAFE;
}
...
static void pc(char **s, size_t *c, size_t *l, char ch) {
  char b[2] = {ch, 0};
  push(s, c, l, b);
}
...
static void adversarial_line(char **s, size_t *c, size_t *l) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (ri(0, 4) == 0) { // image
      push(s, c, l, "![alt](");
      push(s, c, l, (ri(0, 1) ? "https://x/u.png" : "x"));
      push(s, c, l, ")");
      if (ri(0, 1))
        push(s, c, l, "{w=160 h=120 a=0.9 align=right}");
    } else {
      switch (ri(0, 3)) {
      case 0:
        push(s, c, l, P[ri(0, (int)(sizeof(P) / sizeof(*P) - 1))]);
        break;
      case 1:
        push(s, c, l, "word");
        break;
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < iters; i++) {
    ...
    Document d = {0};
    assert(markdown_to_json(md, &d) == 0);
    ...
    char *md1 = NULL;
    assert(json_to_markdown(&d, &md1) == 0 && md1);
    Document d2 = {0};
    assert(markdown_to_json(md1, &d2) == 0);
    char *md2 = NULL;
    assert(json_to_markdown(&d2, &md2) == 0 && md2);
    if (strcmp(md1, md2) != 0) {
