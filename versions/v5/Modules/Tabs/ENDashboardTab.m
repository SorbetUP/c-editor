...
- (NSString*)generateDashboardContent {
    ...
    // Tableau de démonstration Markdown
    [content appendString:@"## 📊 Aperçu Markdown\n\n"];
    [content appendString:@"| Syntaxe | Résultat attendu | Commentaire |\n"];
    [content appendString:@"|:--------|:-----------------|:------------|\n"];
    [content appendString:@"| `[ ] Tâche ouverte` | Liste sans puce `[]` | Vérifie le rendu des cases isolées |\n"];
    [content appendString:@"| `- Élément classique` | Puce standard | Supporte `-`, `*`, `+` |\n"];
    [content appendString:@"| `(1. Élément)` | Numérotation `(1.)` | Parenthèse d'ouverture acceptée |\n"];
    [content appendString:@"| `1] Élément` | Numérotation `1]` | Variante sans parenthèse liée |\n\n"];
